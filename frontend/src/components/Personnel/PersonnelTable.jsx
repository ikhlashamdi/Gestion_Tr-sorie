...
export default function PersonnelTable({
  ...
  const getSortedItems = () => {
    const sortableItems = [...personnels];
    if (sortConfig.key) {
  ...
  const filteredPersonnels = getSortedItems();
    ...
    <div className="px-6 py-6 bg-white rounded-xl shadow-sm">
      {showHeader && (
        <>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-800 mb-2">Gestion des Personnels</h2>
              <p className="text-gray-600">Liste des personnels enregistrés</p>
            </div>
          ...
          {showFilters && (
            <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Trier par</label>
                  <select className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredPersonnels.length === 0 ? (
              <tr>
                <td colSpan="3" className="px-4 py-6 text-center text-gray-500">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <p className="text-gray-600 font-medium">Aucun personnel trouvé</p>
              ...
              filteredPersonnels.map((personnel) => (
                <tr key={personnel._id} className="hover:bg-purple-50 transition-colors">
      ...
      <div className="mt-6 flex justify-between items-center">
        <div className="text-sm text-gray-500">
          {filteredPersonnels.length} personnel(s) affiché(s)
        </div>
      </div>
    </div>
  );
